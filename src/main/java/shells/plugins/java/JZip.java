...
public class JZip implements Plugin {
   ...
   private void load() {
      if (!this.loadState) {
         try {
            InputStream inputStream = this.getClass().getResourceAsStream(String.format("assets/%s.classs", "JZip"));
   ...
   private void zipButtonClick(ActionEvent actionEvent) {
      ...
      if (this.compressDestFileTextField.getText().trim().length() > 0 && this.compressSrcDirTextField.getText().trim().length() > 0) {
         ...
         JOptionPane.showMessageDialog((Component)null, "请检查是否填写完整", "提示", 1);
      }
